...
Daftar ini berlaku pada evidence cut-off 26 Agustus 2026 03:09 WIB dan harus diperbarui
...
| GAP-015 | SagaWork | NEEDS CONFIRMATION | Exact `e2a0391` aktif sebagai public synthetic trial di `app.sagawork.site`: 67 table/26 migration/32 trigger, OpenAPI 76/95/52, 41 file/150 test, 49-page build, manifest 1.141 file, encrypted backup/restore, authenticated/public smoke, dan rollback dua arah PASS. Effective-dated assignment, scoped HR context, GPS setup preview, shift template dan schedule eligibility aktif. F19 rest-day exchange khusus, roster automation, employer-ordered overtime, generalized notification/outbox, XLSX, rights workflow, external provider/legal/device/offsite/independent review/signed admission dan owner/canary UAT masih terbuka. Tidak ada real Staff/photo/GPS dan akun trial tidak boleh menjadi akun pilot. Password trial lemah, photo workflow publik, K0/K1 tetap blocker. | Andreas menjalankan owner feature review dan mencatat revisi. Jangan masukkan data Kananta sebelum F37-F44 lengkap dan signed admission PASS. Tutup backlog F19 dan partial F09/F10/F12/F14/F15/F17/F27/F32/F34-F40, rotasi/hapus akun trial, siapkan protected pilot access/dedicated DB/private provider/offsite backup/hosted CI/legal/device/incident evidence, lalu F49-F52 canary. |
| GAP-016 | SagaView | TODO | Guard retensi S288, rollback inactive, dan deploy backend/Owner exact `8d84c60...` sudah production-verified. S291 exact `1237ef2...` kini menjadi kandidat kumulatif S289+S290 yang `PUSHED / LOCAL_VALIDATED`, migration nol, serta mempunyai dua salinan immutable provenance. Production belum berubah. Authenticated operator UAT belum dieksekusi karena approval serta akun referensi non-customer tetap gate terpisah. | Jalankan release preflight/backup/restore/rollback/smoke sebelum deploy kandidat. Setelah Andreas memberi izin dan akun referensi non-customer tersedia, jalankan harness untuk login Owner, lima layar, dan dua API; lanjutkan katalog/harga/order, stale conflict, recovery/export, serta Support Hub tanpa foto/path customer sebelum menilai `PRODUCTION_ACTIVATED` dan `BUSINESS_READY`. |
...
| GAP-018 | Saga Member Platform | NEEDS CONFIRMATION | DEC-138 dan release `20260902T1526Z-f763fc1-2eaa353` menutup blocker deployment D0 pada private VPS. DEC-139 menambahkan protected Vercel shell exact `c8c776407160c1af7692a068f6a3930ac6ea5b16` / `dpl_6QdcYS8XUTTjV7v7tfQ4SL211Q73`; remote build, headers, exact-asset hash dan browser UAT lulus tanpa backend/API bisnis. Aktivasi R0 tetap tertutup karena exact domain, DNS/TLS, public HTTPS backend route, Resend production, hashed internal allowlist dan expiring passport belum tersedia. Snapshot bridge hanya diterima untuk internal alpha; swap shared host juga perlu dipantau. | Pilih exact domain, konfigurasi DNS/TLS dan Resend melalui channel restricted, buat public HTTPS backend route yang tetap fail-closed, hashed allowlist serta expiring passport, lalu ulangi remote UAT/monitor/rollback sebelum mengubah state menjadi activated. Jangan aktifkan public app, Gateway/QRIS, Push, SagaBook live, NFC, printer, outlet kedua, commercial tenant atau R3-R6. |
