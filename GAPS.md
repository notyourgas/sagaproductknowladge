...
Daftar ini berlaku pada evidence cut-off 26 Agustus 2026 02:14 WIB dan harus diperbarui
...
| GAP-016 | SagaView | TODO | Guard retensi S288, rollback inactive, dan deploy backend/Owner exact `8d84c60...` sudah production-verified. Harness S289 exact `701d008...` sudah `PUSHED / LOCAL_VALIDATED`. S290 exact `f34eff0...` juga menutup focus containment Support Hub pada desktop/mobile tanpa mengubah production atau SagaBook. Authenticated operator UAT belum dieksekusi karena approval serta akun referensi non-customer tetap gate terpisah. | Setelah Andreas memberi izin dan akun referensi non-customer tersedia, jalankan harness untuk login Owner, lima layar, dan dua API; lanjutkan katalog/harga/order, stale conflict, recovery/export, serta Support Hub tanpa foto/path customer sebelum menilai `PRODUCTION_ACTIVATED` dan `BUSINESS_READY`. |
