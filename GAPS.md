...
| GAP-015 | SagaWork / SagaPOS HR | NEEDS CONFIRMATION | Source `c6e2382` menambah pemulihan login/pergantian password staf serta permission/scoped visibility. Lokal 53 file/359 test, lint/type/contract/audit, Linux build, native MySQL concurrency/visibility, full HTTP absensi staf dan Chromium workforce continuation/recovery/Axe/mobile PASS; final harness/docs `fc0062a`. Tidak ada deployment run ini: shared private `0254822` dan public trial `711ef77` tetap. M1 joint browser historis core `aab15ad` + POS `7d328ab` PASS; UI M2 staf native POS belum diuji. DEC-190 tetap arah satu SagaPOS untuk usaha sendiri dengan prioritas Kopi Saga F&B; pilot eksternal lama bukan admission aktif. Program 36 sprint belum selesai seluruhnya, payroll belum operasional, performance OFF. | Koordinasikan M2 staf POS, workflow/permission matrix, mapping outlet/operator, perangkat/GPS/foto nyata, recovery dua database dan deployment. Tetapkan scope/admission internal baru serta aturan gaji aktual bersama Andreas/finance. Acceptance browser workforce tidak menggantikan UI M2 SagaPOS; hasil sintetis tidak menggantikan acceptance nyata atau deployment. |
