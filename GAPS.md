...
| GAP-015 | SagaWork | NEEDS CONFIRMATION | Synthetic staging kini mencakup atomic CSV import, guarded Staff lifecycle, per-staff report + deterministic Summary/Detail CSV, safe PWA install/update baseline, paid/unpaid break, default-off unscheduled reconciliation, supervisor-assisted attendance, dan private non-PII photo-byte pipeline pada same-VPS loopback store. Restricted runtime/OpenAPI artifacts sudah checksum/provenance-verifiable. Implemented data drift gate mencakup 56 tabel/21 migration; validated OpenAPI 3.1.1 mencakup exact 56 path/69 operasi, 39 request components, auth/parameter/idempotency/envelope/error/CSV, source drift verifier, dan pinned independent lint. External SDK/complex response DTO tidak dipublikasikan karena API tetap protected same-origin. Internal disposition `PILOT_READY_CANDIDATE`, tetapi business readiness tetap false karena private source remote/hosted CI, external provider/DPA/browser endpoint, offsite key escrow, legal/privacy, real-device PWA-install/GPS/camera UAT, alert/incident rehearsal, pricing/WTP, named pilot, public route, dan final activation belum lulus. XLSX/async private export, dedicated coverage/leave/exception report, rehire/offboarding authority, scheduled/backdated termination, training/test isolation, serta multi-session policy juga belum lengkap. | Provision private source remote + protected hosted CI, external provider non-PII spike + DPA/key/quota/failure evidence, recovery/key approval, legal sign-off, device/human UAT, named pilot evidence, offboarding authority/SOP, lalu final Go/No-Go Andreas; public API/SDK hanya dibuat bila ada compatibility policy eksplisit, sedangkan XLSX/async export, rehire, employer-ordered/holiday OT, training/test isolation, dan multi-session diputuskan sesuai kebutuhan pilot. |
