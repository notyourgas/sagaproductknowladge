...
| GAP-017 | Snap and Fit | NEEDS CONFIRMATION | Encrypted event-scoped face search dan official liveness client sudah `LOCAL_VALIDATED`, tetapi isolated provider runtime, legal notice approval, real-device UAT, deletion evidence, threshold benchmark, dan transitive React peer compatibility belum terbukti. | Sediakan isolated AWS/Cognito/S3/MySQL/Redis test environment, approved legal notice, dependency compatibility evidence, dan perangkat UAT; jalankan benchmark, privacy deletion, failure/lockout, serta Go/No-Go terpisah sebelum activation. |
