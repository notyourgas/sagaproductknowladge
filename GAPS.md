...
| GAP-015 | SagaWork | NEEDS CONFIRMATION | Synthetic staging kini mencakup atomic CSV import, guarded Staff lifecycle, per-staff report + deterministic Summary/Detail CSV, safe PWA install/update baseline, paid/unpaid break, default-off unscheduled reconciliation, supervisor-assisted attendance, dan private non-PII photo-byte pipeline pada same-VPS loopback store. Restricted runtime/OpenAPI/admission artifacts sudah checksum/provenance-verifiable. Implemented data drift gate mencakup 56 tabel/21 migration; validated OpenAPI 3.1.1 mencakup exact 56 path/69 operasi dan 39 request components. Fail-closed Ed25519 admission control exact `3bcdf06`/`a36ac8e` mengikat source/runtime/preview, maksimum tujuh hari, dan 16 Gate A/B/C/E evidence; 12 synthetic cases PASS, tetapi belum ada real receipt. Internal disposition `PILOT_READY_CANDIDATE`, namun business readiness tetap false karena private source remote/hosted CI, external provider/DPA/browser endpoint, offsite key escrow, legal/privacy, real-device PWA-install/GPS/camera UAT, alert/incident rehearsal, pricing/WTP, named pilot, public route, receipt nyata, dan final activation belum lulus. XLSX/async private export, dedicated coverage/leave/exception report, rehire/offboarding authority, scheduled/backdated termination, training/test isolation, serta multi-session policy juga belum lengkap. | Provision private source remote + protected hosted CI, external provider non-PII spike + DPA/key/quota/failure evidence, recovery/key approval, legal sign-off, device/human UAT, named pilot evidence, public route/incident rehearsal, dan seluruh 16 restricted evidence; Andreas lalu menandatangani exact-target receipt sebelum activation change window. Public API/SDK hanya dibuat bila ada compatibility policy eksplisit; scope export/rehire/OT/training/multi-session diputuskan sesuai kebutuhan pilot. |
