...
- S409 exact `f2198df3e548d2ca54856bc8e6df17d38a36bb0c` menutup risiko
  dialog Blokir Waktu Calendar yang kehilangan aksi pada layar pendek, memakai
  outer scroll, dapat menahan klik tombol tutup, dan membuang draft kotor tanpa
  konfirmasi. Dialog kini memakai header/footer stabil, satu middle scroll,
  aksi 44 piksel, dirty-draft confirm, focus trap/restore, label form, serta
  dukungan forced-colors dan reduced-motion. Matriks 360x740, 375x667,
  393x851, 430x932, 768x1024, dan 1440x900 bebas document overflow; browser
  gabungan 88 pass/11 expected skip, full PHP 1.351/1.351 (15.133 assertion),
  build, design audit 26/0, dan audit dependency nol advisory lulus. Tidak ada
  perubahan API, database, permission, tenant, atau mutation bisnis. Status
  ...
  `20260901155248-9ebdcf1`, rollback `20260901131847-88fd517`, dan
...
- Audio notifikasi admin S385 aktif pada exact merged main
  ...
  `20260831025235-58e1303` yang tersedia dan kompatibel.
  ...
  terbuka karena fitur ini bukan native push. Fresh encrypted backup/checksum/
  disposable restore, exact-commit release gate, verifier, report canary
  32/32, service/journal, migration pending 0, dan public/security smoke 3/3
  lulus tanpa exception. Status `PRODUCTION_DEPLOYED / PRODUCTION_ACTIVATED /
  AUDIO_UAT_PENDING / BUSINESS_READY=false`; UAT speaker nyata masih
  diperlukan.
...
- Exact cumulative source `b37db0810f64731e46f18d026452a6cf3f65e86c`
  ...
  checksum, capacity preflight, disposable restore, full release gate,
  verifier independen 23/23, kontrak post-switch 37/37, report canary 32/32,
  migration pending 0, service/journal, dan public/security smoke 3/3 lulus.
...
- Closing Staff S359/S362 feature source
  ...
  dan diberi alasan; nilai fisik nol juga dapat dikonfirmasi secara eksplisit.
  Preflight menampilkan blocker konkret, submit mengarahkan ke input pertama
  yang perlu diperbaiki, dan blocker ledger keras menyediakan tautan menuju
  pengaturan stok. Backend, workspace Staff, laporan Owner, teks, PNG, dan
  pemulihan draft memakai kontrak yang sama. Ledger tetap server-authoritative,
