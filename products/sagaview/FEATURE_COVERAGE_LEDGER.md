...
Evidence cut-off: 13 Agustus 2026 04:18 WIB

Owner Changelog production-lineage cut-off: 13 Agustus 2026 04:18 WIB. S198
source `ff0c178fe84b36d02fc530a051b0ebc4588715c0`, berbasis exact backend
production `475db4c2`, menambahkan pencarian versi/isi, filter jenis
perubahan, tepat satu detail terbuka, empty-state recovery, dan tautan kembali
ke Galeri Frame dengan tenant/admin-tab tetap utuh. Focused red-green 1/1 pada
1440x900 dan 390x844, regresi navigasi terisolasi 1/1, keyboard Enter, target
kontrol minimal 44 piksel, no-overflow, before/after visual, build 5.097 modul,
npm audit 0/311, diff check, serta review no-upload lulus. Chunk Owner bertambah
4.367 byte mentah / 949 byte gzip. Status `UIUX_VALIDATED / QA_VALIDATED /
SECURITY_VALIDATED / LOCAL_VALIDATED / IMPLEMENTED_NOT_DEPLOYED /
COMMITTED_LOCAL / PUSHED`; production, API, database, payment,
foto/path/output, dan produk lain tidak berubah.
...
| Workspace identity dan Owner Changelog | Owner, operator | Owner Dashboard Changelog, Galeri Frame, dan Studio shell | Changelog responsif dan dapat dicari/difilter; satu detail terbuka; hasil kosong dapat dipulihkan; kembali ke Galeri Frame mempertahankan tenant/tab; shell membedakan workspace administratif dari brand editable; `Cloud vN` dijelaskan per workspace | State pencarian/filter lokal tanpa API; detail pengganti menutup detail lama; konfigurasi aktivasi server-authoritative tetap memperbarui label shell | Route Owner authenticated dan bundle Studio; tidak ada upload foto/path/output atau API baru | Tidak ada migration/storage baru; repair identitas exact-match/idempoten lama tetap satu field setting, version/checksum, dan audit | Tenant route/membership existing; link memakai konteks admin; target repair identity tetap unik dan expected-value match | S198 red-green Changelog 1/1, navigasi 1/1, keyboard/44px/no-overflow, visual 2 viewport, build 5.097 modul, npm audit 0/311, diff check; bukti S140 backend/Studio tetap berlaku | Owner 1440x900 + 390x844; authenticated akun nyata dan monitor/zoom luas residual untuk S198 | Production S140 backend `c2a05076` / Studio `10bcaaae`; kandidat S198 `ff0c178fe84b36d02fc530a051b0ebc4588715c0` | Production `RELEASED / PRODUCTION_ACTIVATED`; S198 `LOCAL_VALIDATED / IMPLEMENTED_NOT_DEPLOYED / COMMITTED_LOCAL / PUSHED` | Guarded deploy dan authenticated Owner UAT pencarian/filter/kembali, plus aktivasi dua akun untuk revision Cloud tanpa silang brand/frame. |
...
| Estimasi total biaya | Owner, operator, customer; attacker synthetic | Studio General/Review, Owner session report, `POST /api/sagadev/sessions/{id}/finish` | General dan Review hanya memakai bahasa estimasi; tidak ada switch pembayaran, QRIS, rekening, paid, provider, expiry, atau gate export/cetak | Total dihitung lokal dan dikirim sebagai `estimatedTotalRupiah`; payload API lama ditolak 422; persisted key Studio lama dibuang saat rehydrate | Finish menerima estimasi 0-100 juta; route mark-paid tidak tersedia; response hanya memuat estimasi | Kolom additive `estimated_total_rupiah`; legacy backend field dinetralkan; state Studio lama disanitasi tanpa mengubah setting operator yang sah | Device/session/tenant guard existing; no-upload tetap; entitlement/action payment lama tidak tersedia | Backend S159 44/44, 741 assertion; S182 focused 10/10 dan full Studio 203/203; S183 browser hydration; S184 browser alias; S186 Playwright Review 7/7 tanpa pengecualian kontras; build/budget/lint/typecheck/audit hijau | General/Review 390x844 dan 1440x900; zoom 200%, no-overflow, forced-colors/reduced-motion, fokus, export aktif, Axe WCAG 2 AA termasuk `color-contrast` lulus | Backend `38c4221e7a37fc3400488d0422bde6440b96ae2f`; Studio aktif `6d7083a3e9ae8e91b948622f24485a4226748344`; kandidat S186 `556c2818c7629907717e245863a3ed2d86649fbe` | `UIUX_VALIDATED / ACCESSIBILITY_VALIDATED / SECURITY_VALIDATED / DATA_INTEGRITY_VALIDATED / QA_VALIDATED / LOCAL_VALIDATED / IMPLEMENTED_NOT_DEPLOYED` | Guarded deploy terpisah dengan backup/restore, exact release, rollback, smoke, serta authenticated UAT estimasi/export/rehydrate sebelum activation/business readiness. |
