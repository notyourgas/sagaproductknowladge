...
memeriksa ulang custody `preflight.json`, `manual-checklist.json`, dan
`evidence-review.json` sebelum snapshot JSON. Seluruh file harus tetap di
evidence root, bebas ancestor reparse point, dan bukan NTFS hard link;
perubahan setelah review S345 berhenti fail-closed dengan blocker spesifik.

Ledger lulus: regression merah 3/3 lalu hijau 3/3; focused S344-S361 67/67;
full 73 file/325 test pada exact commit; parser Windows PowerShell/pwsh;
format/lint/typecheck; client 2.129 modul + SSR 203 modul; bundle 312,7 KiB
dari 450 KiB; npm audit nol; diff check; worktree bersih; serta remote exact.
Uji filesystem sintetis/disposable membuktikan hard link file kontrol dan
junction evidence root ditolak sementara sumber eksternal tidak berubah.
Boundary hanya tooling/test/runbook, tanpa UI/API/database, foto/path customer,
deploy, atau mutasi production. Belum lulus: authenticated manual UAT 12 gate
dan visual review. Status `PUSHED / LOCAL_VALIDATED /
IMPLEMENTED_NOT_DEPLOYED`; `BUSINESS_READY=false`.

S360 Finalize evidence link revalidation: exact pushed source
`5cbe82027896b012086b92fdda38f4c9099cb954` membuat S344 Finalize
memeriksa ulang containment, seluruh ancestor reparse point, dan NTFS hard
link tepat sebelum snapshot/hash evidence final dibaca. Substitusi evidence
setelah review S345 kini berhenti fail-closed dengan blocker spesifik, meski
byte dan hash file pengganti identik.

Ledger lulus: regression merah 2/2 lalu hijau 3/3; focused S344-S360
61/61; full 72 file/322 test pada exact commit; parser Windows
...
serta remote exact. Uji filesystem sintetis/disposable membuktikan hard link
dan junction nyata ditolak sementara sumber eksternal tidak berubah. Boundary
hanya tooling/test/runbook, tanpa UI/API/database, foto, path customer, deploy,
atau mutasi production. Belum lulus: authenticated manual UAT 12 gate dan
visual review. Status `PUSHED / LOCAL_VALIDATED /
IMPLEMENTED_NOT_DEPLOYED`; `BUSINESS_READY=false`.
...
S344, atau reviewer S345 menulis receipt yang sama secara bersamaan. Writer
mengambil lock file eksklusif sebelum membuat temporary/backup atau mengganti
destination; proses kedua berhenti fail-closed dan receipt aktif tetap utuh.
Lock dipegang sampai cleanup atau rollback selesai lalu dihapus oleh handle
Windows `DeleteOnClose`.

Ledger lulus: regression merah 4/4 lalu hijau 4/4; gabungan S352-S358 30/30;
full 70 file/314 test pada exact commit; parser Windows PowerShell/pwsh;
format/lint/typecheck; client 2.129 modul + SSR 203 modul; bundle 312,7 KiB
dari 450 KiB; npm audit nol; diff check; worktree bersih; serta remote exact.
Uji memakai filesystem sintetis/disposable tanpa foto, path, atau output
customer. Boundary hanya tooling/test/runbook, tanpa UI/API/database atau
mutasi production. Belum lulus: authenticated manual UAT 12 gate dan visual
review. Status `PUSHED / LOCAL_VALIDATED / IMPLEMENTED_NOT_DEPLOYED`;
`BUSINESS_READY=false`.

S357 UAT receipt post-switch rollback: exact pushed source
`621fe0025437cf00cc232d506b3ebed3647c83fe` memulihkan kondisi aman bila
verifikasi receipt sesudah atomic switch gagal. Bila receipt lama ada, backup
dikembalikan secara atomik dan byte hasil restore diverifikasi ulang. Bila itu
receipt pertama, destination baru yang belum terverifikasi dihapus dengan guard
path/file yang sama. Error write asli tetap diteruskan agar kegagalan terlihat.

Ledger lulus: regression merah 5/5 lalu hijau 5/5; focused S70/S344-S357
58/58; full 69 file/310 test pada exact commit; parser Windows
PowerShell/pwsh; format/lint/typecheck; client 2.129 modul + SSR 203 modul;
bundle 312,7 KiB dari 450 KiB; npm audit nol; diff check; worktree bersih;
serta remote exact. Uji memakai filesystem sintetis/disposable tanpa foto,
path, atau output customer. Boundary hanya tooling/test/runbook, tanpa UI/API/
database atau mutasi production. Belum lulus: authenticated manual UAT 12 gate
dan visual review. Status `PUSHED / LOCAL_VALIDATED /
IMPLEMENTED_NOT_DEPLOYED`; `BUSINESS_READY=false`.

S356 UAT receipt post-switch integrity: exact pushed source
`e624bf4395996663816e168cbf0a90ebc4fd4692` mengunci lalu membaca kembali
receipt tujuan sesudah atomic move/replace dan membandingkan setiap byte dengan
payload yang sudah di-flush. Backup receipt lama baru dihapus setelah verifikasi
ini lulus; bila destination berubah atau rusak, writer berhenti fail-closed dan
backup lama tetap tersedia untuk recovery.

Ledger lulus: regression merah 4/4 lalu hijau 5/5; gabungan S352-S356 21/21;
focused S70/S344-S356 47/47; full 68 file/305 test pada exact commit; parser
Windows PowerShell/pwsh; format/lint/typecheck; client 2.129 modul + SSR 203
modul; bundle 312,7 KiB dari 450 KiB; npm audit nol; diff check; worktree
bersih; serta remote exact. Uji memakai filesystem sintetis/disposable tanpa
foto, path, atau output customer. Boundary hanya tooling/test/runbook, tanpa
UI/API/database atau mutasi production. Belum lulus: authenticated manual UAT
12 gate dan visual review. Status `PUSHED / LOCAL_VALIDATED /
IMPLEMENTED_NOT_DEPLOYED`; `BUSINESS_READY=false`.
...
merupakan Windows junction/symlink/reparse point sebelum writer membuat file
sementara. Receipt lama yang berupa reparse point atau NTFS hard link juga
ditolak sebelum atomic replace. Guard berlaku konsisten pada S70, S344, dan
reviewer S345; foto, path customer, atau output tidak pernah dipakai.

Ledger lulus: regression merah 4/4 lalu hijau 5/5 dengan junction dan hard
link disposable; focused S70/S344-S353 54/54; full 65 file/292 test; parser
Windows PowerShell/pwsh; format/lint/typecheck; client 2.129 modul + SSR 203
modul; bundle 312,7 KiB dari 450 KiB; npm audit nol; diff check; worktree
bersih; serta remote exact. Boundary hanya tooling/test/runbook, tanpa UI/API/
database atau mutasi production. Belum lulus: authenticated manual UAT 12 gate
dan visual review. Status `PUSHED / LOCAL_VALIDATED /
IMPLEMENTED_NOT_DEPLOYED`; `BUSINESS_READY=false`.
...
tercakup atribut reparse point. Evidence hard-linked ditahan per gate;
preflight dan checklist hard-linked ditolak sebelum parsing atau hashing.

Ledger lulus: regression merah membuktikan scanner lama menerima file luar
root melalui hard link, lalu hijau; focused S344-S349 21/21, full 279/279,
parser PowerShell/pwsh, format/lint/typecheck, client+SSR build, bundle budget,
npm audit nol, diff check, serta remote exact. Boundary tetap offline tanpa
API/database atau mutasi production. Belum lulus: authenticated manual UAT 12
gate, visual review, dan S344 Finalize. Status `PUSHED / LOCAL_VALIDATED /
IMPLEMENTED_NOT_DEPLOYED`; production tetap S343 dan `BUSINESS_READY=false`.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Authenticated UAT reduced diagnostic S366 | Owner UAT non-customer; Production Auditor | Wrapper S289; Owner Console read-only | Tidak ada UI runtime baru | Early failure dan payload tidak lengkap menghasilkan receipt status/count/boolean allowlist sebelum throw | Browser hanya boleh GET/HEAD/OPTIONS; guard tetap menolak mutasi, data customer, foto, dan path lokal | File receipt lokal ditulis atomik; tanpa migration/database/customer storage | Metadata akun dan approval harus cocok; credential tetap lewat bridge loopback dan tidak dicetak | Preflight synthetic menulis 1 receipt tanpa marker sensitif/temp; UAT final fail-closed karena safety flag `null`, count `-1` | UAT real production read-only; visual tidak dinilai lulus karena runner berhenti | Backend `b2467cd9cf3165fdfc3f6488dbedbc45fc7e6f59`; focused 8/108; full 1.034/13.237; build 5.097; audit nol; verifier v3 ready | `PUSHED / LOCAL_VALIDATED / IMPLEMENTED_NOT_DEPLOYED / UAT_BLOCKED` | Perbaiki schema early-failure runner dengan fixture sintetis; ulang authorized UAT. Deploy tetap dilarang sampai empat safety flag boolean `false`. |
