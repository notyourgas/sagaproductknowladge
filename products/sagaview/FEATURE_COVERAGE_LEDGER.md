...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Support Hub path privacy S294 | Owner sah; accidental local-path input | `POST /api/admin/support/ask` menuju hub pusat | Tidak ada perubahan UI | Pertanyaan tetap terkirim, tetapi path lokal diganti `[LOCAL_PATH]`; metadata di luar allowlist dibuang | Redaksi terjadi sebelum request jaringan SagaView; tidak ada foto/path/output customer yang diteruskan | Tidak ada migration; remote-mode test tidak membuat conversation atau attachment lokal | Session Owner, tenant, tab, HMAC gateway, dan product scope existing; perubahan hanya saat `productCode=sagaview` | Red test membuktikan path Windows keluar; green membuktikan redaksi dan metadata allowlist; focused 46/253 serta full 193/1.588 lulus | N/A backend/privacy; UI existing tidak berubah | Source `be0d730758c8678c263a8ea12ed09924db89e23a`; build 5.097, Pint/diff, Composer/npm audit nol; archive `844542a2...`, bundle `d37b4305...` dua salinan | `PUSHED / SECURITY_VALIDATED / QA_VALIDATED / LOCAL_VALIDATED / IMPLEMENTED_NOT_DEPLOYED` | Guarded deploy exact candidate lalu authenticated Owner ask/handoff UAT sebelum activation/business readiness. |
| Support Hub bootstrap boundary S293 | Owner sah; anonymous negative | `GET /api/admin/support/bootstrap` | Tidak ada perubahan visual/runtime | Fresh tab authenticated mendapat bootstrap kosong; tanpa sesi gagal tertutup | Response mempertahankan `productCode=sagaview` dan `sagaViewPhotosLocalFirst=true`; tidak ada foto/path/output | Tidak membuat conversation atau attachment; tanpa migration/storage baru | Tanpa sesi/konteks ditolak; sesi Owner dan tenant context existing wajib | Anonymous negative dan fresh-tab authenticated 200 lulus; DB conversation/attachment tetap nol | N/A test-only; browser production authenticated masih residual | Source `cdc49b51c816fadda9298d924164e0a87c048000`; focused 1/7, gabungan 37/176, build 5.097, audit nol; production S292 `1237ef2d...` tetap aktif | `PUSHED / SECURITY_VALIDATED / QA_VALIDATED / LOCAL_VALIDATED / IMPLEMENTED_NOT_DEPLOYED` | Jalankan authenticated production UAT terotorisasi dengan akun referensi non-customer sebelum activation/business readiness. |
