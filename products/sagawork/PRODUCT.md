...
Evidence status: `CONFIRMED / SYNTHETIC_TRIAL_LOGIN_PASS / OWNER_FEATURE_REVIEW_READY`
...
- Active isolated Hostinger staging sekarang exact runtime `c14f5c2`, MySQL 8.4 dengan 66 application table/24 migration/32 trigger, dan hanya data sintetis. Feature `eb3b372` plus acceptance `0cd3516` menambahkan satu identity HR trial allowlisted yang langsung aktif tanpa TOTP hanya pada exact `staging` + `synthetic` + organisasi `synthetic-*`; identitas dan password tidak dipublikasikan. Secure bootstrap reguler tetap tersedia dan akun HR/Owner/Payroll pilot atau production tetap wajib TOTP.
- Full gate terbaru lulus 38 test file/137 test dan 47-page build. Direct login menghasilkan privileged session; overview, people, scheduling, requests, exceptions, reports, settings, dan account mengembalikan HTTP 200, sedangkan password salah HTTP 401. Loopback memakai cookie HttpOnly/SameSite=Strict khusus trial karena belum ada TLS publik; cookie `__Host-` Secure tetap default untuk akun reguler.
- Backup sebelum/sesudah migration, restore rehearsal dengan 24 tombstone dan zero resurrection, serta guarded rollback `4eb01c3 → d36a59f → 4eb01c3` lulus. Artifact lintas OS awal gagal memuat dependency Linux dan otomatis kembali ke runtime lama; release kemudian dibangun ulang di Linux dan tervalidasi aktif. Host `/tmp` masih sekitar 99% terpakai sehingga tetap menjadi risiko operasi shared-VPS.
- Pre-provision encrypted backup menyimpan 24 tombstone; schema tidak berubah. Linux manifest memverifikasi 1.084 file dan guarded rollback `c14f5c2 → eb3b372 → c14f5c2` lulus. Build Linux pertama berhenti sebelum activation karena working directory salah; runtime lama tetap aktif, corrected build kemudian lulus.
- Pilot topology belum dipasang dan akun trial bukan akun pilot. External private-evidence, protected TLS, dedicated pilot database, provider/offsite recovery, signed admission, serta actual human feature review belum lengkap; `K0_ENVIRONMENT_READY=NO_GO` dan `K1_HR_LOGIN_READY=NO_GO`. Tidak ada real Staff data, foto, GPS, production activation, atau perubahan Vercel Preview.
