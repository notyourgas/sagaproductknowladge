...
Evidence status: `CONFIRMED / SYNTHETIC_STAGING_DEPLOYED / OWNER_ACTIVATION_PENDING`
...
- Active isolated Hostinger staging sekarang exact runtime `4eb01c3`, MySQL 8.4 dengan 66 application table/24 migration/32 trigger, dan hanya data sintetis. Provisioner fail-closed pada exact `staging` + `synthetic`, domain email non-deliverable, token hash-only maksimal 24 jam, serta handoff root-only. Satu akun HR demo sintetis menunggu aktivasi Owner; identitas dan secret tidak dipublikasikan.
