...
Evidence status: `CONFIRMED / SPRINT_31_LOCAL_VALIDATED / STAGING_UNCHANGED`
...
- Sprint 31 exact feature source `6d3fc98` dan acceptance `666fbef` menambah guarded/idempotent first-organization + HR provisioner, one-time bootstrap activation, HR-owned password, mandatory TOTP, server-versioned policy acknowledgement, serta root-only operator recovery yang merotasi MFA dan mencabut sesi lama. Local gate lulus 36 test file/130 test, 47-page build, 66-table/24-migration contract, OpenAPI 70 path/85 operasi/48 request components, dan disposable Hostinger MySQL integration; database/grant/source temporary telah dibersihkan.
- Pilot topology belum dipasang. Operator cleanup menutup blocker disk lama (read-only observation sekitar 43% terpakai/56 GiB bebas), tetapi swap pressure dan external private-evidence, protected access, dedicated pilot database, backup/restore, serta rollback evidence belum lengkap; `K0_ENVIRONMENT_READY=NO_GO` dan `K1_HR_LOGIN_READY=NO_GO`. Tidak ada akun, credential, real Staff data, foto, GPS, runtime staging, atau production yang berubah.
...
- Implemented-contract local Sprint 31 mencakup seluruh 66 tabel/24 migration. Validated OpenAPI 3.1.1 mencakup exact 70 path/85 operasi, 48 request components, auth/parameter/idempotency/envelope/error/CSV, source drift verifier, dan pinned independent lint. Active synthetic staging tetap pada kontrak runtime lama 65 tabel/23 migration sampai deployment terpisah; external SDK/response DTO tidak dipublikasikan karena API tetap protected same-origin.
