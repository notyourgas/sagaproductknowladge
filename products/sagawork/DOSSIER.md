...
Next.js 16 PWA, React 19, TypeScript, Drizzle, MySQL 8.4, Node 22, systemd, isolated Hostinger staging, dan Vercel Preview. Active synthetic staging exact `c14f5c2` mempunyai 66 application table/24 migration/32 trigger; full gate meluluskan 38 test file/137 test, 47-page build, OpenAPI 70 path/85 operasi/48 request components, encrypted pre-provision backup, manifest 1.084 file, dan guarded rollback `c14f5c2 → eb3b372 → c14f5c2`. Protected Vercel Preview tetap `dpl_9zvZTjgQBRhHJm5pVXH4rmtqQaBg` `READY` tanpa perubahan.
...
Handoff source baseline `d36a59f` tetap memiliki restricted reproducible archive dan CycloneDX 1.6 SBOM yang tervalidasi; artifact restricted tidak disalin ke knowledge publik. Provenance trial terbaru adalah feature `eb3b372`, loopback-session fix/runtime `c14f5c2`, dan acceptance `0cd3516`. Satu identity HR trial aktif tanpa TOTP hanya bila email+organisasi cocok exact runtime allowlist pada staging sintetis; credential dan identity tidak masuk knowledge publik. Password tetap Argon2, rate limit/lockout/generic 401/audit/session cap tetap aktif. Cookie loopback non-Secure dibatasi ke boundary yang sama; akun reguler mempertahankan `__Host-` Secure cookie dan mandatory TOTP.
...
Andreas menerima jalur Sprint 30–40. Exact local source `a500a52` menutup bagian code Sprint 30 dengan fail-closed runtime/topology untuk `pilot_restricted`. Secure bootstrap `6d3fc98` dan synthetic demo `4eb01c3` tetap tersedia; direct-trial slice `eb3b372`/`c14f5c2` kini memberi Owner akses feature review tanpa activation/TOTP pada satu identity allowlisted. Delapan area HR HTTP 200, wrong password 401, backup, manifest, health, dan rollback lulus. Ini bukan akun pilot dan tidak berisi real employee data. `/tmp` shared host masih hampir penuh; protected pilot access, dedicated pilot database, external provider/offsite recovery, signed evidence, serta human feature review belum lengkap sehingga K0/K1 tetap `NO_GO`.
