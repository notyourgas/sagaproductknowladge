...
Next.js 16 PWA, React 19, TypeScript, Drizzle, MySQL 8.4, Node 22, systemd, isolated Hostinger staging, dan Vercel Preview. Active runtime `da2e662` memiliki 21 migration, 56 application table, 26 append-only trigger, dan manifest 26 artifact. 25 test file/84 unit test, smoke 17 area, security abuse 7/7, browser+Axe 12 pass/4 intentional scope skip, 720-request load, recovery, timer, dan guarded rollback `da2e662 → b3c36cf → da2e662` lulus pada synthetic staging. Protected Staff/HR layouts dipaksa dynamic sehingga build tanpa database environment tidak dapat memublikasikan workspace statis.
