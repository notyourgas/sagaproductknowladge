...
Next.js 16 PWA, React 19, TypeScript, Drizzle, MySQL 8.4, Node 22, systemd, isolated Hostinger staging, dan Vercel Preview. Active runtime `e003a3d` memiliki 21 migration, 56 application table, 26 DBA-visible append-only trigger, dan manifest 26 artifact. 27 test file/89 unit test, smoke 18 area, security abuse 7/7, browser+Axe 12 pass/4 intentional scope skip, 720-request load, recovery, timer, dan guarded rollback `e003a3d → 9d49565 → e003a3d` lulus pada synthetic staging. Protected Vercel Preview `dpl_CxS7HN4JT7SQaKASqcfegtTsxAZF` `READY` dengan 39-page build; authenticated health, manifest, dan safe-worker contract PASS tanpa MySQL. Protected Staff/HR layouts dipaksa dynamic sehingga build tanpa database environment tidak dapat memublikasikan workspace statis.
...
Handoff source untuk exact runtime `e003a3d` memiliki restricted tracked-file archive yang reproducible dari Git, immutable provenance sidecar, dan SHA-256 sidecar. Artifact berukuran 1.176.243 byte dengan checksum `461efe3105ebfbebb743ade95f59ad0cb70fbcb89ebd2655d00845dd3cd7db34`. OpenAPI implementation/acceptance exact `85bbbfb` mempunyai artifact terpisah 1.213.154 byte/444 tracked entries dengan checksum `9deb21651fb89dca6dbeee25e3f5626d88937f0bc0d38c8055adaf31120c61f9`. Verifier memastikan artifact tidak memuat secret, runtime env, database dump, PII, dependency tree, atau build cache; file restricted tidak disalin ke repository knowledge publik. Dokumentasi implementasi memetakan 56 tabel/21 migration. OpenAPI 3.1.1 machine-readable exact-set mencakup 56 path/69 operasi, 39 request components, cookie/public auth boundary, path/query/header/idempotency, stable envelope/error/CSV, generated artifact freshness, dan source drift verifier; pinned Redocly CLI lint PASS tanpa warning. Complex success projection tetap internal dan external SDK tidak dipublikasikan selama API protected same-origin.
...
Positioning sementara: workforce operations action-first dan evidence-based untuk tim shift Indonesia, bukan HRIS enterprise lengkap. Pricing/SLA `NEEDS CONFIRMATION`. Internal disposition tetap `PILOT_READY_CANDIDATE`; private source remote/hosted CI, external provider/DPA, offsite/key custody, legal/privacy, real-device install/update/GPS/camera behavior, incident alert, offboarding authority/rehire, employer-ordered overtime/holiday-rest scope, named pilot, willingness-to-pay, dan real signed admission receipt tetap blocker eksternal. Produk tidak boleh diklaim payroll compliant, closed-beta active, atau production-ready.
