...
dengan Instrument Serif untuk display dan Plus Jakarta Sans variable self-hosted
untuk seluruh UI/body storefront serta admin; wordmark COYABAG tetap memakai
font logo resminya.
...
- Detail retur mengarahkan keputusan item karantina ke antrean Inventory yang
  guarded. Restock atau disposal memerlukan review dampak, alasan, recent-auth,
  ledger append-only, dan audit trail; aksi langsung dari detail retur dihapus.
...
- Halaman pembayaran menampilkan QRIS provider yang tervalidasi langsung di
  ...
  provider ke server, bukan dari membuka atau memindai gambar.
...
- Integrasi storefront tidak lagi meminta content key global
  ...
  digunakan pada route yang memang membutuhkannya. Plus Jakarta Sans kini aktif
  dan terverifikasi pada seluruh UI/body storefront serta admin; Instrument
  Serif tetap dipakai untuk display editorial.
...
- Release historis `20260901-d09960c` dengan exact material source
  ...
  Release lebih lama `20260830-22bb066` tetap memuat promo baru yang selalu
  ...
  menjadi paid. Detail pembayaran gateway kini juga menampilkan sepuluh hasil
  pencocokan terbaru dengan run, scope, provider, nominal pesanan, nominal
  teramati, selisih, dan waktu. Snapshot terenkripsi serta summary run tidak
  diteruskan ke browser; nominal provider yang hilang tidak ditampilkan sebagai
  nol dan selisih tetap fail-closed. Eligibility pemeriksaan provider kini
  server-authoritative: retry hanya tersedia untuk transaksi provider aktif
  yang pending atau membutuhkan recovery. Matched, simulasi, transfer manual,
  provider nonaktif, dan amount mismatch tertahan di backend sekaligus UI;
  mismatch diarahkan ke antrean exception sebelum pemeriksaan baru. Pengiriman
  memakai tarif manual yang siap pada sembilan kota. Shipment Detail kini
  memakai dialog aksesibel untuk exception dan delivery; status `delivered`
  ...
  stok fisik serta karantina tanpa menambah available. Alert stok rendah kini
  membuka antrean triase dengan severity, saldo
  tersedia, batas minimum, kekurangan menuju saldo aman, waktu pemicu, dampak
  ...
  yang eligible; satu guard basi atau item tidak eligible menolak seluruh
  batch tanpa mutasi parsial. Finance tidak menerima kontrol maupun akses
  route mutasi. Detail Pesanan juga
  ...
  Beranda Admin kini dapat membandingkan tren terautentikasi 7, 14, atau 30
  hari. Pilihan tervalidasi backend, tersimpan di URL, mempunyai state
  loading/success/error aksesibel, dan mempertahankan snapshot valid terakhir
  saat refresh gagal. Grafik panjang bergulir di dalam panel pada mobile tanpa
  melebarkan halaman.
  Admin Pesanan kini dapat menyimpan maksimal lima tampilan antrean/filter per
