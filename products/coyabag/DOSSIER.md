...
Eligibility action kini berasal dari backend dan diperiksa ulang oleh endpoint:
pending/recovery pada provider aktif dapat diperiksa, sedangkan matched,
simulasi, manual, provider nonaktif, dan amount mismatch tetap fail-closed.
Action recovery tampil kontekstual di panel rekonsiliasi pada desktop/mobile;
amount mismatch diarahkan ke exception review dan tidak memanggil provider.
...
lineage source/reversal. Reversal hanya tersedia untuk movement eligible dan
tetap membuat movement lawan; record ledger tidak diedit atau dihapus.

Triase stok rendah menampilkan severity, available, threshold, kekurangan ke
saldo aman, waktu pemicu, dampak storefront, alert sebelumnya/berikutnya,
