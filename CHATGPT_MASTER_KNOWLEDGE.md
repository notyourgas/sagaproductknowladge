...
SagaWork active synthetic staging runtime sekarang exact `c14f5c2`; direct-trial feature `eb3b372`, acceptance `0cd3516`. Satu restricted allowlisted HR trial identity dapat login tanpa TOTP hanya pada exact `staging` + `synthetic` + organisasi sintetis. Credential/identity tidak dipublikasikan; password tetap Argon2, rate limit/lockout/generic 401/audit/session cap tetap aktif. Loopback HTTP memakai fallback cookie HttpOnly/SameSite=Strict hanya pada boundary trial; standard `__Host-` Secure cookie dan mandatory TOTP tetap berlaku untuk HR/Owner/Payroll reguler, pilot, dan production. Full gate 38 file/137 test, 47-page build, 66 application table/24 migration/32 trigger, OpenAPI 70/85/48; eight HR areas HTTP 200, wrong password 401, encrypted backup 24 tombstone, manifest 1.084 file, health, dan rollback `c14f5c2 → eb3b372 → c14f5c2` PASS. Owner feature review siap, tetapi account ini tidak boleh dipromosikan; pilot harus memakai database, email, token, password, dan TOTP baru. Shared-host `/tmp` hampir penuh; protected pilot TLS, external provider/offsite recovery, dedicated topology, signed admission, serta human UAT belum lulus sehingga K0/K1 tetap NO-GO. Vercel Preview dan production tidak berubah; performance pilot OFF dan payroll hanya payroll-ready.
